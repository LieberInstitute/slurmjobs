...
test_that("array_submit_restore") {
    temp = run_test(basic_job, submit = FALSE, task_ids = 1:3, restore = TRUE)
    expect_equal(temp$original, temp$final)
}

test_that("array_submit_numeric_task_ids") {
    temp = run_test(basic_job, submit = FALSE, task_ids = 1:3, restore = FALSE)
...
test_that("array_submit_broken_script") {
    expect_error(
        run_test(broken_job, submit = FALSE, task_ids = 1:3, restore = FALSE),
...
#   found
test_that("array_submit_null_task_ids") {
    expect_error(
